...
export class AuthService {
  ...
  async validateClinicUser(identifier: string, password: string): Promise<any> {
    const user = await this.userRepository.findOne({
      where: [
        { email: identifier, role: UserRole.CLINIC },
        { phone: identifier, role: UserRole.CLINIC },
      ],
    });
  ...
  async loginClinic(identifier: string, password: string) {
    const user = await this.validateClinicUser(identifier, password);
    const payload = { email: user.email, sub: user.id, role: user.role };
